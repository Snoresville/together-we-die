...
CONTAINERS_DEBUG = false -- Should we print debugging prints for containers
...
--[[
  ...
  Container Creation:
    Containers:CreateContainer({
      ...
      headerText         = "#lootbox",
      draggable          = true,
      position           = "200px 200px 0px",  -- "30% 40%"   -- "mouse"    -- "entity" -- "css"
      equipment          = true,
      range              = 250,
      closeOnOrder       = true,
      forceOwner         = false,
...
function Containers:start()
  ...
  CustomNetTables:SetTableValue("containers_lua", "use_panorama_inventory", {value=true})
  ...
  -- Shooper: Disabling the bottom timer as I don't think it does anything
  --          The stuff in the class is just for shops right now
...
function Containers:OrderFilter(order)
  ...
  if order.units["0"] and #conts > 0 then
    ...
    if container then
      -- SWAT Custom - Sniper's ITD must not trigger this because he needs to be able
      -- to mark ABMs to telepawn to. In general, I feel we should not override the action
      -- if the unit is using a spell that targets the ABM
      if order.entindex_ability and order.entindex_ability > 0 then
          return true
      end
  ...
  if order.units["0"] and order.order_type == DOTA_UNIT_ORDER_PURCHASE_ITEM then
    ...
    if defInventory then
      ...
      g_GoldManager:spendGoldForPlayerID(ownerID, cost)
...
function Containers:Containers_OnSell(args)
  ...
  item:RemoveSelf()
  g_GoldManager:giveGoldToPlayerID(playerID, cost)
...
function Containers:CreateShop(cont)
  ...
  -- Custom SWAT code for valor
  if cont.valorPrices then
      for k,v in pairs(cont.valorPrices) do
        local item = k
        if type(k) ~= "number" then
          item = k:GetEntityIndex()
        end
  ...
  function shop:BuyItem(playerID, unit, item)
    ...
    local valorPrice = self:GetValorPrice(item)
    local stock = self:GetStock(item)
    local owner = PlayerResource:GetSelectedHeroEntity(playerID)

    local gold = PlayerResource:GetGold(playerID) + g_GoldManager.goldPoolAmount
    if valorPrice > 0 then
        -- This item requires valor
        local playerInfo = unit.playerInfo
        if not playerInfo then
            Containers:DisplayError(playerID, "#valor_buy_fail_nonofficer")
        elseif valorPrice > (playerInfo.valor + g_ValorManager.valorPoolAmount) then
            Containers:DisplayError(playerID, "#valor_buy_fail_not_enough_valor")
        else
            g_ValorManager:spendValorForPlayerID(playerID, valorPrice)
            ...
            g_StatsManager:incrementStatForPlayerId(playerID, PlayerStatsManager.VALOR_SPENT, valorPrice)

            return newItem
        end
    elseif gold >= cost and (stock == nil or stock > 0) then
      local newItem = CreateItem(item:GetAbilityName(), owner, owner)
      ...
      g_GoldManager:spendGoldForPlayerID(playerID, cost)

      g_StatsManager:incrementStatForPlayerId(playerID, PlayerStatsManager.CREDITS_SPENT, cost)
  ...
  function shop:GetValorPrice(item)
    item = GetItem(item)
    return PlayerTables:GetTableValue(ptID, "valorPrice" .. item:GetEntityIndex()) or -1
  end
  ...
  -- SWAT Custom addition to show amount of time until restock
  function shop:GetRestockTime(item)
    item = GetItem(item)
    return PlayerTables:GetTableValue(ptID, "restockTime" .. item:GetEntityIndex())
  end
  function shop:SetRestockTime(item, restockTime)
    item = GetItem(item)
    if restockTime and restockTime > 0 then
      PlayerTables:SetTableValue(ptID, "restockTime" .. item:GetEntityIndex(), restockTime)
    else
      PlayerTables:DeleteTableKey(ptID, "restockTime" .. item:GetEntityIndex())
    end
  end
  function shop:GetRestockMaxTime(item)
    item = GetItem(item)
    return PlayerTables:GetTableValue(ptID, "restockMaxTime" .. item:GetEntityIndex())
  end
  function shop:SetRestockMaxTime(item, restockMaxTime)
    item = GetItem(item)
    if restockMaxTime and restockMaxTime > 0 then
      PlayerTables:SetTableValue(ptID, "restockMaxTime" .. item:GetEntityIndex(), restockMaxTime)
    else
      PlayerTables:DeleteTableKey(ptID, "restockMaxTime" .. item:GetEntityIndex())
    end
  end
...
function Containers:CreateContainer(cont)
  ...
  local pt =
    {id =          self.nextID,
     ...
     usesColumns = cont.usesColumns,
     usesCustomChildren = cont.usesCustomChildren,
