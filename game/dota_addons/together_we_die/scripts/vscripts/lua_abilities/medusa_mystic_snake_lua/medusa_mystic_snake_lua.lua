...
function medusa_mystic_snake_lua:OnSpellStart()
    ...
    local base_damage = self:GetSpecialValueFor("snake_damage") + math.floor(self:GetCaster():GetAgility() * self:GetSpecialValueFor("agi_multiplier"))
    local mult_damage = self:GetSpecialValueFor("snake_scale") / 100
    -- Talent Tree
    self.silence_duration = 0.0
    local special_mystic_snake_silence_lua = self:GetCaster():FindAbilityByName("special_mystic_snake_silence_lua")
    if special_mystic_snake_silence_lua and special_mystic_snake_silence_lua:GetLevel() ~= 0 then
        self.silence_duration = special_mystic_snake_silence_lua:GetSpecialValueFor("value")
    end
    -- Talent Tree
    self.stun_duration = 0.0
    local special_mystic_snake_stun_lua = self:GetCaster():FindAbilityByName("special_mystic_snake_stun_lua")
    if special_mystic_snake_stun_lua and special_mystic_snake_stun_lua:GetLevel() ~= 0 then
        self.stun_duration = special_mystic_snake_stun_lua:GetSpecialValueFor("value")
    end
...
function medusa_mystic_snake_lua:OnProjectileHit_ExtraData(target, location, ExtraData)
    ...
    if target and (not target:IsMagicImmune()) and (not target:IsInvulnerable()) then
        ...
        if self.silence_duration ~= 0 then
            target:AddNewModifier(
                    self:GetCaster(), -- player source
                    self, -- ability source
