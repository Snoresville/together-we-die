...
function modifier_antimage_mana_break_lua:OnCreated( kv )
	...
	self.mana_agi_multiplier = self:GetAbility():GetSpecialValueFor( "mana_agi_multiplier" )
	self.damage_agi_multiplier = self:GetAbility():GetSpecialValueFor( "damage_agi_multiplier" )
end
...
function modifier_antimage_mana_break_lua:GetModifierProcAttack_BonusDamage_Physical( params )
	if IsServer() and (not self:GetParent():PassivesDisabled()) then
		...
		if result == UF_SUCCESS then
			...
			local mana_break = self.mana_break + agility * self.mana_agi_multiplier
			local mana_burn =  math.min( target:GetMana(), mana_break )
			target:ReduceMana( mana_burn )

			self:PlayEffects( target )

			return mana_burn * (self.mana_damage_pct + agility * self.damage_agi_multiplier)
		end
