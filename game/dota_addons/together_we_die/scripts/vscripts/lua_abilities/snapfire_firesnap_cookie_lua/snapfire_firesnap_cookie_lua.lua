...
function snapfire_firesnap_cookie_lua:OnProjectileHit(target, location)
    ...
    if target:IsChanneling() or target:IsOutOfGame() then
        return
    end
    ...
    local damage = self:GetSpecialValueFor("impact_damage") + self:GetCaster():GetStrength() * self:GetSpecialValueFor("str_multiplier")
    ...
    local callback = function()
        ...
        -- cast spell
        local snapfire_scatterblast = self:GetCaster():FindAbilityByName("snapfire_scatterblast_lua")
        ...
        for _, enemy in pairs(enemies) do
            ...
            if (snapfire_scatterblast and snapfire_scatterblast:GetLevel() ~= 0) then
                self:GetCaster():SetCursorPosition(enemy:GetOrigin())
                snapfire_scatterblast:OnSpellStart()
            end
        end
