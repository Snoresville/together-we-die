...
function modifier_slardar_corrosive_haze_lua:OnCreated( kv )
	-- references
	self.armor_reduction = self:GetAbility():GetSpecialValueFor( "armor_reduction" ) - self:GetAbility():GetSpecialValueFor( "str_multiplier" ) * self:GetCaster():GetStrength()
...
function modifier_slardar_corrosive_haze_lua:PlayEffects()
	...
	self.effect_cast = assert(loadfile("lua_abilities/rubick_spell_steal_lua/rubick_spell_steal_lua_arcana"))(self, particle_cast, PATTACH_OVERHEAD_FOLLOW, self:GetParent() )
