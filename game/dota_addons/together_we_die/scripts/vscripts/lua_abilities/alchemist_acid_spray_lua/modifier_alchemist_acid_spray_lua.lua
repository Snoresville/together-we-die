...
function modifier_alchemist_acid_spray_lua:OnCreated( kv )
	...
	local damage = self:GetAbility():GetSpecialValueFor( "damage" ) + (self:GetCaster():GetStrength() * self.str_multiplier)
	self.armor = -self:GetAbility():GetSpecialValueFor( "armor_reduction" ) + (self:GetCaster():GetStrength() * self.armor_str_multiplier)
