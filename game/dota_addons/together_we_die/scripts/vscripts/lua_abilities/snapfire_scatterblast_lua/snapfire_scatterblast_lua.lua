...
function snapfire_scatterblast_lua:OnProjectileHit_ExtraData(target, location, extraData)
    ...
    local point_blank_mult = self:GetSpecialValueFor("point_blank_dmg_bonus_pct")
    -- Talent Tree
    local special_scatterblast_dmg_bonus_pct_lua = caster:FindAbilityByName("special_scatterblast_dmg_bonus_pct_lua")
    if special_scatterblast_dmg_bonus_pct_lua and special_scatterblast_dmg_bonus_pct_lua:GetLevel() ~= 0 then
        point_blank_mult = point_blank_mult + special_scatterblast_dmg_bonus_pct_lua:GetSpecialValueFor("value")
    end
    -- divide by 100 to get multiples from percentage
    point_blank_mult = point_blank_mult / 100
    local str_multiplier = self:GetSpecialValueFor("str_multiplier")
    -- Talent Tree
    local special_scatterblast_str_multiplier_lua = caster:FindAbilityByName("special_scatterblast_str_multiplier_lua")
    if special_scatterblast_str_multiplier_lua and special_scatterblast_str_multiplier_lua:GetLevel() ~= 0 then
        str_multiplier = str_multiplier + special_scatterblast_str_multiplier_lua:GetSpecialValueFor("value")
    end
    local damage = self:GetSpecialValueFor("damage") + math.floor(caster:GetStrength() * str_multiplier)
