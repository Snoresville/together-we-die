...
--------------------------------------------------------------------------------
-- AOE Radius
function windranger_shackleshot_lua:GetAOERadius()
...
function windranger_shackleshot_lua:OnProjectileHit_ExtraData(target, location, ExtraData)
    ...
    local damage = self:GetCaster():GetIntellect() * self:GetSpecialValueFor("int_multiplier")
    local damageTable = {
        victim = nil,
