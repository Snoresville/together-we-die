...
function modifier_doom_devour_lua:OnCreated( kv )
	...
	self.gold_str_multiplier = self:GetAbility():GetSpecialValueFor( "gold_str_multiplier" )
end
...
function modifier_doom_devour_lua:OnDestroy()
	...
	if self:GetParent():IsAlive() then
		local totalBonusGold = self.bonus_gold + self:GetParent():GetStrength() * self.gold_str_multiplier
