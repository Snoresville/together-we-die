...
function modifier_mars_bulwark_lua:OnCreated(kv)
    ...
    if IsServer() then
        self.permanent_health_modifier = self:GetParent():FindModifierByName("modifier_mars_bulwark_lua_gain")
        if not self.permanent_health_modifier then
            self.permanent_health_modifier = self:GetParent():AddNewModifier(
                    self:GetParent(),
                    self:GetAbility(),
                    "modifier_mars_bulwark_lua_gain",
                    {}
            )
        end

        self.permanent_health_modifier:ForceRefresh()
    end
end
...
function modifier_mars_bulwark_lua:OnRemoved()
    if IsServer() then
        self:GetParent():RemoveModifierByName("modifier_mars_bulwark_lua_gain")
    end
end
...
function modifier_mars_bulwark_lua:GetModifierPhysical_ConstantBlock(params)
    ...
    if angle_diff < self.angle_front then
        ...
        -- Increase health when successfully block from front
        if self:GetAbility():IsFullyCastable() then
            self.permanent_health_modifier:SetStackCount(self.permanent_health_modifier:GetStackCount() + 1)
            self.permanent_health_modifier:ForceRefresh()
            -- cooldown
