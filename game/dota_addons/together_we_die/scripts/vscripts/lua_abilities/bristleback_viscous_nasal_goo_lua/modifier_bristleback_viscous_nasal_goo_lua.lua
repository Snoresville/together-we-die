...
function modifier_bristleback_viscous_nasal_goo_lua:OnCreated( kv )
	...
	self.slow_base = self:GetAbility():GetSpecialValueFor( "base_move_slow" ) + (self:GetCaster():GetStrength() * self:GetAbility():GetSpecialValueFor("str_multiplier"))
	self.slow_stack = self:GetAbility():GetSpecialValueFor( "move_slow_per_stack" ) + (self:GetCaster():GetStrength() * self:GetAbility():GetSpecialValueFor("str_multiplier"))
