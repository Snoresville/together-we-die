...
function modifier_furion_wrath_of_nature_thinker_lua:OnCreated( kv )
	self.damage = self:GetAbility():GetSpecialValueFor( "damage" ) + (self:GetCaster():GetIntellect() * self:GetAbility():GetSpecialValueFor("int_multiplier"))
	...
	self.damage_scepter = self:GetAbility():GetSpecialValueFor( "damage_scepter" ) + (self:GetCaster():GetIntellect() * self:GetAbility():GetSpecialValueFor("int_multiplier"))
...
function modifier_furion_wrath_of_nature_thinker_lua:HitTarget( hTarget )
	...
	if bHasScepter then
		...
		if ( not hTarget:IsAlive() ) and hForceOfNature ~= nil and hForceOfNature:IsTrained() then
			local hForceOfNatureLevel = hForceOfNature:GetLevel()
			local treantName = "npc_dota_furion_holdout_treant_" .. hForceOfNatureLevel
			if hTarget:IsHero() then
				treantName = "npc_dota_furion_holdout_treant_large"
			end
