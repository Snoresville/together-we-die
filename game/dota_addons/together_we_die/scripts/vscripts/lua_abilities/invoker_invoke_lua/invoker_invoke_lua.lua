...
--------------------------------------------------------------------------------
function invoker_invoke_lua:GetCooldown(level)
...
function invoker_invoke_lua:OnUpgrade()
    self.max_invoked_spells = self:GetSpecialValueFor("max_invoked_spells")
    -- add orb manager
...
function invoker_invoke_lua:OnHeroCalculateStatBonus()
    if self.ability_manager and #self.ability_manager.ability_slot == 0 then
        -- add empty ability
...
function invoker_invoke_lua:GetOrbLevel(orb_name)
    if not self.orb_manager.status[orb_name] then
        return 0
    end
...
function ability_manager:InitAbility(ability)
    ...
    ability.GetOrbSpecialValueFor = function(self, key_name, orb_name)
        ...
        if not self.orbs[orb_name] then
            return 0
        end
