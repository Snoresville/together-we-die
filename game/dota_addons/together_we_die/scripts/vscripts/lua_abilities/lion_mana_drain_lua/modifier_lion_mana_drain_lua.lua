...
function modifier_lion_mana_drain_lua:OnCreated( kv )
	...
	self.mana = self:GetAbility():GetSpecialValueFor( "mana_per_second" ) + int_multiplier * self:GetCaster():GetIntellect()
...
function modifier_lion_mana_drain_lua:OnIntervalThink()
	...
	-- check distance
	if (self:GetParent():GetOrigin()-self:GetCaster():GetOrigin()):Length2D()>self.break_distance then
		self:Destroy()
		return
	end
