...
function modifier_lifestealer_open_wounds_lua:OnCreated(kv)
    -- references
    self.heal = self:GetAbility():GetSpecialValueFor("heal_percent") / 100 + self:GetCaster():GetStrength() * self:GetAbility():GetSpecialValueFor("str_multiplier") -- special value
    self.step = 1
