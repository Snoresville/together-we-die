...
function modifier_invoker_chaos_meteor_lua_thinker:OnCreated( kv )
	if IsServer() then
		...
		self.distance = self:GetAbility():GetOrbSpecialValueFor( "travel_distance", "w" ) + self:GetCaster():GetIntellect() * self:GetAbility():GetOrbSpecialValueFor( "int_multiplier", "w" )/10
		...
		local damage = self:GetAbility():GetOrbSpecialValueFor( "main_damage", "e" ) + self:GetCaster():GetIntellect() * self:GetAbility():GetOrbSpecialValueFor( "int_multiplier", "e" )
