...
function modifier_queen_of_pain_shadow_strike_lua:OnCreated( kv )
	-- references
	local intDamage = self:GetCaster():GetIntellect() * self:GetAbility():GetSpecialValueFor( "int_multiplier" )
	self.max_slow = self:GetAbility():GetSpecialValueFor( "movement_slow" )
	local init_damage = self:GetAbility():GetSpecialValueFor( "strike_damage" ) + intDamage
	local tick_damage = self:GetAbility():GetSpecialValueFor( "duration_damage" ) + intDamage
