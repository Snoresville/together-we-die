...
LinkLuaModifier( "modifier_furion_teleportation_lua", "lua_abilities/furion_teleportation_lua/modifier_furion_teleportation_lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------

function furion_teleportation_lua:OnAbilityPhaseStart()
	self.vTargetPosition = self:GetCursorPosition()
	self.int_multiplier = self:GetSpecialValueFor("int_multiplier")
...
function furion_teleportation_lua:OnSpellStart()
	...
	local damageTable = {
		...
		damage = math.floor(self.int_multiplier * self:GetCaster():GetIntellect()),
		damage_type = self:GetAbilityDamageType(),
		ability = self, --Optional.
	}

	-- Find Units in Radius and apply damage
	local enemies = FindUnitsInRadius(
			self:GetCaster():GetTeamNumber(), -- int, your team number
			self.vTargetPosition, -- point, center point
