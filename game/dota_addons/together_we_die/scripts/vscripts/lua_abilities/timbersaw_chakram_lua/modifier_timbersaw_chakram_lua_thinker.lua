...
function modifier_timbersaw_chakram_lua_thinker:OnCreated( kv )
	...
	-- references
	self.damage_pass = self:GetAbility():GetSpecialValueFor( "pass_damage" ) + (self.caster:GetStrength() * self:GetAbility():GetSpecialValueFor( "str_multiplier" ))
	self.damage_stay = self:GetAbility():GetSpecialValueFor( "damage_per_second" ) + (self.caster:GetStrength() * self:GetAbility():GetSpecialValueFor( "str_multiplier" ))/2
	self.radius = self:GetAbility():GetSpecialValueFor( "radius" )
	self.speed = self:GetAbility():GetSpecialValueFor( "speed" )
	self.duration = self:GetAbility():GetSpecialValueFor( "pass_slow_duration" )
	self.manacost = self:GetAbility():GetSpecialValueFor( "mana_per_second" ) + (self.caster:GetStrength() * self:GetAbility():GetSpecialValueFor( "str_multiplier" ))/20
