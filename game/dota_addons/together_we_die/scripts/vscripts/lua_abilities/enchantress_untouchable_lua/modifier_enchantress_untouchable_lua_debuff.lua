...
function modifier_enchantress_untouchable_lua_debuff:OnCreated( kv )
	...
	self.damage_reduction = self:GetAbility():GetSpecialValueFor( "damage_reduction" ) - self:GetCaster():GetIntellect() * self:GetAbility():GetSpecialValueFor( "int_multiplier" )
end
...
function modifier_enchantress_untouchable_lua_debuff:GetModifierPreAttack_BonusDamage()
	if IsServer() then
		if self:GetParent():GetAggroTarget()==self:GetCaster() then
