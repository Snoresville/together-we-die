...
--------------------------------------------------------------------------------
modifier_creature_dark_willow_bedlam_lua = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_creature_dark_willow_bedlam_lua:IsHidden()
	return false
end

function modifier_creature_dark_willow_bedlam_lua:IsDebuff()
	return false
end

function modifier_creature_dark_willow_bedlam_lua:IsPurgable()
	return false
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_creature_dark_willow_bedlam_lua:OnCreated( kv )
	...
	self.wisp:AddNewModifier(
		self:GetCaster(), -- player source
		self:GetAbility(), -- ability source
		"modifier_creature_dark_willow_bedlam_lua_attack", -- modifier name
		{} -- kv
	)
...
function modifier_creature_dark_willow_bedlam_lua:OnRefresh( kv )
	-- refresh references
	self.revolution = self:GetAbility():GetSpecialValueFor( "roaming_seconds_per_rotation" )
	self.rotate_radius = self:GetAbility():GetSpecialValueFor( "roaming_radius" )
...
function modifier_creature_dark_willow_bedlam_lua:OnRemoved()
end

function modifier_creature_dark_willow_bedlam_lua:OnDestroy()
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_creature_dark_willow_bedlam_lua:OnIntervalThink()
	-- update position
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_creature_dark_willow_bedlam_lua:PlayEffects()
	-- Get Resources
