...
function modifier_enchantress_natures_attendants_lua:GetAttributes()
...
function modifier_enchantress_natures_attendants_lua:OnCreated( kv )
	...
	self.int_multiplier = self:GetAbility():GetSpecialValueFor( "int_multiplier" ) -- special value
...
function modifier_enchantress_natures_attendants_lua:OnIntervalThink()
	...
	local n = #targets
	local total_heal_amt = self.heal + self.int_multiplier * self:GetCaster():GetIntellect()

	-- heal random target
	for i=1,self.count do
		allies[targets[RandomInt(1,n)]]:Heal( total_heal_amt, self:GetAbility() )
	end
end
